...
This only works if we assume the above. In step one we depend that we will always find one digit that fits our constraint. Otherwise, the data is incorrect or we face a completely different problem with repeating digits.
